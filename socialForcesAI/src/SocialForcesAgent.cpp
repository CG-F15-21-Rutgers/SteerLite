...
{
	Util::Vector force = Util::Vector(0, 0, 0);
	SteerLib::AgentInterface * tAgent;
	SteerLib::ObstacleInterface * tOb;
	Util::Vector away = Util::Vector(0, 0, 0);
	Util::Vector aOb = Util::Vector(0, 0, 0);
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++)
	{
		if ((*neighbor)->isAgent())
		{
			tAgent = dynamic_cast<SteerLib::AgentInterface *>(*neighbor);

			Util::Vector awayTemp = normalize(position() - tAgent->position());

			away = away + awayTemp*(_SocialForcesParams.sf_agent_a*exp((((this->radius() + tAgent->radius()) - (this->position() - tAgent->position()).length()) / _SocialForcesParams.sf_agent_b)));
		...
		{
			tOb = dynamic_cast<SteerLib::ObstacleInterface *>(*neighbor);
			Util::Vector wallNormal = calcWallNormal(tOb);
			std::pair<Util::Point, Util::Point> line = calcWallPointsFromNormal(tOb, wallNormal);
			std::pair<float, Util::Point> min_stuff = minimum_distance(line.first, line.second, position());

			Util::Vector aObTemp = normalize(position() - min_stuff.second);

			aOb = aOb + aObTemp * (_SocialForcesParams.sf_wall_a*exp((((this->radius()) - (this->position() - min_stuff.second).length()) / _SocialForcesParams.sf_wall_b)));
...
{
	int constant = 5;  // change constant
	Util::Vector Force = (((_goalDirection * PREFERED_SPEED) - velocity()) / _dt) / constant;
	return Force;
}
...
Util::Vector SocialForcesAgent::calcAgentRepulsionForce(float dt)
{
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++)
	{
		...
		if ((id() != tAgent->id()) && (tAgent->computePenetration(this->position(), this->radius()) > 0.000001))
		{
			agent_repulsion_force = agent_repulsion_force + (tAgent->computePenetration(this->position(), this->radius()) * _SocialForcesParams.sf_agent_body_force * dt) * normalize(position() - tAgent->position());
...
Util::Vector SocialForcesAgent::calcWallRepulsionForce(float dt)
{
	...
	std::set<SteerLib::SpatialDatabaseItemPtr> _neighbors; gSpatialDatabase->getItemsInRange(_neighbors, _position.x - (this->_radius + _SocialForcesParams.sf_query_radius), _position.x + (this->_radius + _SocialForcesParams.sf_query_radius), _position.z - (this->_radius + _SocialForcesParams.sf_query_radius), _position.z + (this->_radius + _SocialForcesParams.sf_query_radius), dynamic_cast<SteerLib::SpatialDatabaseItemPtr>(this));
...
bool SocialForcesAgent::runLongTermPlanning()
{
	...
	//==========================================================================
	AStarPlanner astar;
	if (!astar.computePath(agentPath, pos, _goalQueue.front().targetLocation, gSpatialDatabase, false))
	{
		return false;
	}
